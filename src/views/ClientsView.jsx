...
export default function ClientsView({ searchQuery, onSelectContact }) {
  ...
  const { contacts, isLoading, isSearching, status, totalCount, sentinelRef } = useClientSearch(searchQuery);
      ...
      {isLoading ? (
        ...
        <div className="bg-background-card border border-border rounded-2xl shadow-card py-20 text-center">
          ...
          {isSearching
            ...
            : <button onClick={() => setShowModal(true)} className="mt-3 text-sm text-primary hover:text-primary-hover font-medium transition-colors">Add your first client</button>
          }
        </div>
      ) : (
        <div className="bg-background-card border border-border rounded-2xl shadow-card overflow-hidden">
