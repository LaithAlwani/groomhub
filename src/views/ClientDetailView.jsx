...
export default function ClientDetailView({ contactId, onBack }) {
  ...
  const [vaccPetTab,       setVaccPetTab]       = useState(null);
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      {/* Vaccinations */}
      {(() => {
        const multiplePets  = (pets?.length ?? 0) > 1;
        const activePetId   = multiplePets ? (vaccPetTab ?? pets?.[0]?._id ?? null) : (pets?.[0]?._id ?? null);
        const visibleVaccs  = multiplePets
          ? (vaccinations ?? []).filter((v) => v.pet_id === activePetId)
          : (vaccinations ?? []);
        const today = new Date().toISOString().slice(0, 10);

        function VaccTable({ rows }) {
          return rows.length === 0 ? (
            ...
            <div className="bg-background-card border border-border rounded-2xl overflow-hidden shadow-card">
              ...
              <div className="divide-y divide-border">
                {rows.map((vacc) => {
                  const isOverdue = vacc.due_date && vacc.due_date < today;
                  return (
                    <div key={vacc._id} className="flex items-center gap-3 px-5 py-3 hover:bg-ui-hover transition-colors">
                      ...
                      <div className="w-16 shrink-0 flex items-center justify-end gap-1">
                        {confirmDeleteVaccination === vacc._id ? (
                          <>
                            <button onClick={() => handleDeleteVaccination(vacc._id)} className="text-xs font-medium text-white bg-danger px-1.5 py-0.5 rounded-md">Yes</button>
                            <button onClick={() => setConfirmDeleteVaccination(null)} className="text-xs text-text-muted px-1 py-0.5">✕</button>
                          </>
                        ) : (
                          <>
                            <button onClick={() => setVaccinationModal({ mode: "edit", vaccination: vacc })} className="p-1 text-text-muted hover:text-text-primary hover:bg-ui-active rounded-lg transition-colors">
                              <Icon name="edit" className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => setConfirmDeleteVaccination(vacc._id)} className="p-1 text-text-muted hover:text-danger hover:bg-tag-red rounded-lg transition-colors">
          ...
          <div>
            ...
            {multiplePets && (
              <div className="flex items-center gap-1 mb-3 flex-wrap">
                {pets.map((pet) => {
                  const count = (vaccinations ?? []).filter((v) => v.pet_id === pet._id).length;
                  const isActive = activePetId === pet._id;
                  return (
                    <button
                      key={pet._id}
                      onClick={() => setVaccPetTab(pet._id)}
